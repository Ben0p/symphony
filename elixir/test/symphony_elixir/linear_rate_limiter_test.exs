defmodule SymphonyElixir.LinearRateLimiterTest do
  ...
  setup do
    ...
    tracker_settings = %{
      ...
      provider: %{
        ...
        "rate_limit_max_retry_after_ms" => 3_600_000
      }
    }
    ...
    {:ok, tracker_settings: tracker_settings, state_path: state_path}
  end
  ...
  test "honors a bounded retry-after cooldown", %{tracker_settings: tracker_settings} do
    bounded_settings = put_in(tracker_settings, [:provider, "rate_limit_max_retry_after_ms"], 100)
    assert :ok = RateLimiter.observe_response(bounded_settings, %{status: 429, headers: [{"retry-after", "1"}]})

    started_at = System.monotonic_time(:millisecond)
    assert :ok = RateLimiter.await(bounded_settings)

    assert System.monotonic_time(:millisecond) - started_at >= 75
  end

  test "persists a GraphQL cooldown when the client returns an ok response tuple", %{tracker_settings: tracker_settings, state_path: state_path} do
    response = %{
      status: 400,
      body: %{
        "errors" => [
    ...
    assert {:ok, next_allowed_at} = File.read(state_path)
